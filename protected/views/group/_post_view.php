			...
			<tr>
				...
				<td>
					...
					<?php if($member->inRoles(array('superAdmin','admin'))):?>
						<?php echo CHtml::link(($post->isTop>0?Yii::t('app','取消置顶'):Yii::t('app','置顶')),array('group/setTopPost','id'=>$group->id,'postId'=>$post->id,'value'=>($post->isTop>0?'0':'1')),array('class'=>'muted'));?>
					<?php endif;?>
					&nbsp;|&nbsp;
					<?php 
					if(Yii::app()->user->checkAccess('updatePost',array('post'=>$post))){
						 echo CHtml::link(Yii::t('app','编辑'),array('post/update','id'=>$post->id),array('class'=>'muted'));
					}
					?>
					&nbsp;|&nbsp;
				<?php if(Yii::app()->user->checkAccess('deletePost',array('post'=>$post)) || $member->inRoles(array('superAdmin','admin'))):?>
						<?php echo CHtml::link(Yii::t('app','删除'),array('group/deletePost','id'=>$group->id,'postId'=>$post->id),array('class'=>'muted'));?>
				<?php endif;?>
					</div>
